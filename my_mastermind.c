...
int second_main(char* code, int rounds);
char* generate_code();
...
int second_main(char* code, int rounds) {
    if (is_number_valid(code) || rounds < 1) return 1;
    print_guess_prompt();
    for (int i = 0; i < rounds; ++i) {
        ...
        while (1) {
            ...
            if (is_number_valid(users_input)) {
                ...
                if (checker(code, users_input) == 1) return 0;
                break;
            }
        }
    }
...
int process_user_input(char* code, int rounds) {
    return second_main(code, rounds);
}
...
int main(int argc, char** argv) {
    ...
    int result = process_user_input(Secret_Code, rounds);
    if (Secret_Code != NULL)
        free(Secret_Code);
    return result;
}
